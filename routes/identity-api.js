...
const { makeCode, ensureInteracted } = require('./identity'); // <-- helpers module
...
router.post('/request-code', async (req, res) => {
  try {
    ...
    // TODO: send email/SMS with loginCode
  ...
  try {
    ...
    const m = await query(
      `SELECT member_id, login_code, login_code_expires
         FROM ff_member WHERE interacted_code=$1`,
      [code]
    );
    const row = m.rows[0];
    if (!row || row.login_code !== submitted ||
   ...
   RETURNING u.member_id, i.joined_at
    ...
    await query(
      `INSERT INTO ff_event (interacted_code, member_id, type)
       VALUES ($1, $2, 'verify.click')`,
