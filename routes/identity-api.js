...
const isPhone = (x='') => /^\+?[0-9][0-9\s\-().]{5,}$/.test(x.trim());
  ...
  try {
    ...
    await pool.query(
      ...
      [
        ...
        // store IP hash redundantly for investigation
        crypto.createHash('sha256').update(String(req.headers['cf-connecting-ip'] || req.ip || '')).digest('hex'),
      ]
    );

    // set first_identifier if not captured yet
    await pool.query(
      `UPDATE ff_invite
          SET first_identifier = COALESCE(first_identifier, $1)
        WHERE invite_id = $2`,
      [value, invite?.invite_id || null]
    );

    // Send via your NotificationApi
...
router.post('/api/verify/confirm', async (req, res) => {
  try {
    ...
    const { kind, value } = normalizeIdentifier(rawKind, rawValue);
    if (!kind || !value || !code) {
      return res.status(422).json({ ok:false, error:'bad_request' });
    }
    ...
    // Mark invite joined (and optionally attach member_id if you have it in session)
    const memberId = req.session?.member_id || null; // adapt if you store session differently
    await pool.query(
      `UPDATE ff_invite
    ...
    // Optionally: update ff_members verified_at here
    // if (memberId && kind === 'email') { ... } etc.
