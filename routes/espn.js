...
function ensureMemberId(v) {
  ...
  return crypto
    .randomBytes(8).toString('base64').replace(/[^A-Z0-9]/gi,'').slice(0,8).toUpperCase()
    || 'ABCDEFGH';
}

async function ensureSessionRow(memberId, existingSid = null) {
...
async function upsertEspnCred(swidBrace, s2) {
  ...
  await pool.query(
    `INSERT INTO ff_espn_cred (swid, espn_s2, first_seen, last_seen)
     VALUES ($1::uuid, NULLIF($2,''), NOW(), NOW())
     ON CONFLICT (swid)
...
async function findOrCreateMemberBySwid(swidBrace) {
  ...
  const { rows } = await pool.query(
    `SELECT id, member_id, quick_snap, swid
       FROM ff_quickhitter
      WHERE quick_snap = $1 OR swid = $2::uuid
      ORDER BY last_seen_at DESC NULLS LAST, updated_at DESC NULLS LAST, created_at DESC
      LIMIT 1`,
    [swidBrace, swidUuid]
  );
  ...
  if (rows.length) {
    const row = rows[0];
    const memberId = norm(row.member_id) || ensureMemberId(null);
    // backfill any missing fields + touch last_seen_at
    await pool.query(
      `UPDATE ff_quickhitter
          SET member_id = $1,
              ...
              last_seen_at = NOW()
        WHERE id = $4`,
      [memberId, swidBrace, swidUuid, row.id]
    );
    return memberId;
  }

  // create a new link row
  const memberId = ensureMemberId(null);
  await pool.query(
...
async function setFFSessionCookies(req, res, memberId) {
  const existingSid = req.cookies?.ff_sid || null;
  const sid = await ensureSessionRow(memberId, existingSid);
  cookies.setSessionCookie(res, sid);        // sets ff_sid (HttpOnly, long-lived)
  cookies.setMemberCookie(res, memberId);    // sets ff_member (readable)
...
router.get('/login', async (req, res) => {
  try {
    ...
    if (!swidBrace || !s2) {
      return res.status(400).json({ ok: false, error: 'missing_or_bad_swid_or_s2' });
    }

    // sanitize/resolve redirect target
    ...
    // 1) set ESPN cookies on our domain (short TTL for s2)
    ...
    // 3) find or create member bound to this SWID
    const memberId = await findOrCreateMemberBySwid(swidBrace);

    // 4) set FF session + member cookies
    await setFFSessionCookies(req, res, memberId);

    // 5) (optional) clear espn_s2 on our domain after attestation
    //    comment out if you want to keep it around longer
    cookies.clearEspnS2(res);

    // 6) INGEST — fire-and-forget signal with headers
    try {
      const origin = `${req.protocol}://${req.get('host')}`;
      fetch(`${origin}/api/platforms/espn/ingest/espn/fan`, {
        method: 'POST',
