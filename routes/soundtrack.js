...
async function applyPrivacyToSharedCaptures({ viewerMemberId, ownerMemberId, captures }) {
  if (!ownerMemberId || !Array.isArray(captures) || !captures.length) return captures;
  const zonesMap = await loadZonesForMembers([ownerMemberId], pool);
  const zones = zonesMap.get(ownerMemberId) || [];
  if (!zones.length) return captures;
