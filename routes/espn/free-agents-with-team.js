...
async function espnPlayersPage({ season, leagueId, week, slotIds, offset, limit, teamId, memberId }, req, credCandidate){
  ...
  return { status, arr, url, used, attempts, error };
}
...
async function pullFreeAgentsDirect({ season, leagueId, week, pos, teamId, memberId }, req, credCandidate){
  ...
  const tried   = [];
  const errors  = [];
  let activeCred = credCandidate || null;
  let usedCred   = credCandidate || null;
  let attempts   = [];
  let lastStatus = null;
  let lastError  = null;
  ...
  for (let offset=0; offset<1000; offset+=LIMIT){
    ...
    if (!activeCred && used) activeCred = used;
    if (error) {
      errors.push(error);
      lastError = error;
    }
  ...
  return { rows: all, tried, errors, status: lastStatus, attempts, used: usedCred, error: lastError };
}
