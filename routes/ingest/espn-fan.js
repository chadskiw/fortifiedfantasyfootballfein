...
router.post('/season', async (req, res) => {
  const pool   = req.app.get('pg'); // pg.Pool set in server.js
  const season = Number(req.body?.season);
  if (!Number.isFinite(season)) return res.status(400).json({ ok:false, error:'season required' });

  try {
    // 1) discover leagues from ESPN “poll”
    const poll = await espnGet(req.app, 'poll', { season });

    // store raw poll
    await pool.query(`
      INSERT INTO ff_espn_fan (season, kind, payload)
      VALUES ($1, 'poll', $2::jsonb)
      ON CONFLICT DO NOTHING
    `, [season, poll]);

    const leagues = (poll?.leagues || []).map(l => String(l.leagueId));
    ...
    res.json({ ok:true, season, leaguesCount: leagues.length });
  } catch (err) {
    console.error('[ingest/fan]', err);
