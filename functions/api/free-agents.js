...
const PROTEAM_BY_ID = {
  ...
  // ESPN sometimes aliases:
  31:'BAL',33:'BAL',32:'HOU',34:'HOU'
};
...
// ✅ Authoritative mapping from ESPN *defaultPositionId* (not lineup slots)
...
// Lineup slot ids are still fine as a fetch filter, but NEVER for labeling positions
    ...
    headers: {
      ...
      "x-fantasy-filter": JSON.stringify({
        players: {
          ...
          // OK to restrict ESPN response set by lineup slots, but DO NOT use this to label positions.
          filterSlotIds: { value: SLOT_IDS },
          // A lightweight, stable sort to surface likely targets:
          sortPercOwned: { sortAsc: false, sortPriority: 1 },
          limit: 2000, // ESPN honors this with kona views; harmless otherwise.
        },
      }),
...
export const onRequestGet = async ({ request, env }) => {
  try {
    ...
    for (const pl of raw) {
      ...
      // ✅ FIX: map from *defaultPositionId* → canonical position
      const pos       = POS_BY_DEFAULT_ID[Number(P.defaultPositionId)] || "UTIL";
    ...
    // Filters
    let filtered = base
      // ✅ Drop low-signal rows: Proj < 2 AND FMV is non-positive/invalid
      .filter(p => {
