...
function resolveCreds(req, url) {
  const cookies = parseCookieHeader(req.headers.get("cookie") || "");
  let swid = cookies.SWID || url.searchParams.get("swid") || "";
  let s2   = cookies.espn_s2 || cookies.ESPN_S2 || url.searchParams.get("s2") || "";
...
function opponentFromStats(stats, week, byeWeek) {
  const W = Number(week);
  const rows = Array.isArray(stats) ? stats : [];

  // 1) exact W
  let row = rows.find(s => Number(s.scoringPeriodId) === W) || null;

  // 2) else newest row with scoringPeriodId <= W
  if (!row) {
    const le = rows.filter(s => Number(s.scoringPeriodId) <= W);
    if (le.length) {
      row = le.slice().sort((a, b) => Number(b.scoringPeriodId || 0) - Number(a.scoringPeriodId || 0))[0];
    }
  }
  ...
  let opp =
    ...
    row?.opponent?.abbrev ||
    (Number.isFinite(row?.opponentTeamId) ? toAbbr(row.opponentTeamId) : null) ||
    null;

  if (!opp) return null;

  const abbr = normTeam(opp);
  const isBye = /^BYE/i.test(abbr);

  // Only return 'BYE' if this week is actually a BYE for player team (or row is for W)
  if (isBye) {
    if (Number.isInteger(byeWeek) && byeWeek === W) return 'BYE';
    if (Number(row.scoringPeriodId) === W) return 'BYE';
    return null; // future BYE leaked backward → ignore
  }
  return abbr;
}
...
export const onRequestGet = async ({ request }) => {
  try {
    ...
    // ESPN auth
    const { swid, s2 } = resolveCreds(request, url);
  ...
  .filter(p => !(Number.isFinite(p.fmv) && p.fmv < 1 && p.fmv > 1000));  // ✅ drop players with fmv < 1 (but keep null fmv)
    ...
    return json({
      ...
      meta: {
        ...
        usedByPos,
        slotIds: SLOT_IDS,
        statuses: ["FREEAGENT", "WAIVERS", "ONROSTER"],
        minProj,
