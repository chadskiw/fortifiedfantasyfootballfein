// functions/api/platforms/espn/roster.js
// GET  /api/platforms/espn/roster?season=2025&leagueId=...&week=1[&teamId=4]
// When teamId is present -> single team payload:
//   { ok:true, platform:"espn", leagueId, season, week, teamId, team_name, players:[...] }
// When teamId is absent -> league-wide payload:
//   { ok:true, platform:"espn", leagueId, season, week, teams:[ { teamId, team_name, players } ] }
...
/* --------------------- ESPN --------------------- */
// Build URLs (read host + standard host fallback)
...
const SLOT = {
  ...
  24: "WR/RB",
  25: "WR/TE",
  26: "QB/RB/WR/TE", // SUPERFLEX in some leagues
  27: "OP",       // Another superflex alias
};

// Pro team id -> abbreviation fallback (covers standard ESPN ids)
const PROTEAM = {
...
function normalizeOneTeam(t) {
  // roster shape can vary; prefer t.roster.entries
  ...
  return {
    teamId: String(t?.id ?? ""),
    team_name: teamDisplayName(t), // NOTE: timeline expects 'team_name'
    players,
  };
}
      ...
      return json(
        {
          ...
          hint: "Pass cookies or X-ESPN-SWID/X-ESPN-S2 headers",
        },
        401
      );
    }
