...
// Ensure schema (add swid/s2 if missing)
await (async () => {
  await pool.query(`
  ...
  `);
  // For existing tables, add cols if they don't exist
...
app.post("/fein/upsert-meta", requireKey, async (req, res) => {
  try {
    ...
    // OPTIONAL creds — only store if present
...
app.get("/fein-auth/creds", requireKey, async (req, res) => {
  try {
    ...
    // Return exactly what your CF worker expects:
